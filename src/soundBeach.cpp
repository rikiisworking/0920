...
void soundBeach::drawWaves(float * fft)
{
	...
	ofBeginShape();
	ofVertex(-100 - ofGetWidth() / 2, 1920 - ofGetHeight() / 2);
	for (int i = 0; i < 256; i++) {
		float v = ofMap(fft[fftAddress[i]], 0.0f, 2.5f, 0.0f, 0.6f, true);
		v = ofClamp(v, 0.0f, 0.2f);
		ofVertex(-100 + (i * 10) - ofGetWidth() / 2, 500 + 1200 * v - ofGetHeight() / 2);
		ofVertex(-100 + (i * 10) - ofGetWidth() / 2 + 5.0f, 500 + 1200 * v - ofGetHeight() / 2);
	}
	ofVertex(2020 - ofGetWidth() / 2, 1920 - ofGetHeight() / 2);
	ofEndShape();
	ofPopMatrix();
}
...
void soundBeach::generateParticles()
{
	particles.push_back(
		...
		(int)ofRandom(life, life + 50),
			...
			ofVec2f(ofRandom(0, 1920), 540.0f),
...
void soundBeach::draw2(float * fft, ofVec2f lHand, ofVec2f rHand, ofVec2f head)
{
	...
	float scaleRate = ofMap(ofDist(lHand.x, lHand.y, rHand.x, rHand.y),0,1920.f,0.8f,1.2f,true);
	float movePos = ofMap(head.x, 0, 1920, 200.0f, -200.0f, true);
	ofPushMatrix();
	ofTranslate(ofGetWidth() / 2, ofGetHeight() / 2);
...
bool soundBeach::PositionInScreen(float size, ofVec2f pos) {
	bool temp = true;
	if (pos.x<-size || pos.x>ofGetWidth() + size || pos.y<-size || pos.y>ofGetHeight() + size) {
		temp = false;
	}return temp;
}