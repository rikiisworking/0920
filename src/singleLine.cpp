...
void singleLine::draw(float * amps)
{
	for (int i2 = 0; i2 < 4; i2++){
		...
		for (int i = 0; i < 256; i++) {
			i2%2==0? p[i2].lineTo(ofPoint(i*7.5, -ofClamp(amps[amp[i2][i] - 256 * i2], 0.0f, 1.3f) * 800 + 1080.0f)):
			p[i2].lineTo(ofPoint(i*7.5, +ofClamp(amps[amp[i2][i]-256*i2], 0.0f, 1.3f) *800-30.0f));
		}
	}
	lineShader.begin();
