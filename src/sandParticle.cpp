...
void sandParticle::setup()
{
	...
	for (int i = 0; i < 5000; i++) {
		particlePosition.push_back(ofVec2f(ofRandom(0, 1920), (ofRandom(0, 1080))));
...
void sandParticle::update(float * fft, bool kicked)
{
	for (int i = 0; i < 5000; i++) {
		...
		if (particlePosition[i].x > 1920 || particlePosition[i].x < 0) { velocity[i].set(-velocity[i].x, velocity[i].y); }
		if (particlePosition[i].y > 1080 || particlePosition[i].y < 0) { velocity[i].set(velocity[i].x, -velocity[i].y); }
	}
