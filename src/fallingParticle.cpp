...
void fallingParticle::setup()
{
	...
	for (int i = 0; i < this->number_of_targets; i++) {
		...
		this->circles.back().get()->setup(this->box2d.getWorld(), ofRandom(150.0f, ofGetWidth() - 150.0f), ofRandom(ofGetHeight()), ofRandom(7.0f, 50.0f));
...
void fallingParticle::update(ofPolyline line,bool kicked)
{
	...
	for (int i = 0; i < this->circles.size(); i++) {
		if (kicked) {
			this->circles[i]->addForce(ofVec2f(0, -8.0f), 200.0f);
